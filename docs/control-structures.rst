...
the gas can be specified with special modifiers ``.value()`` and ``.gas()``, respectively::
...
As seen in the example, it is possible to forward Ether to the creation using the ``.value()`` modifier,
